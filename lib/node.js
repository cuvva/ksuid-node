...
function getDockerInstance() {
	...
	try {
		bytes = fs.readFileSync('/proc/1/cpuset', 'utf-8').trim();
	} catch (error) {
		return null; // not a docker container
	}

	if (!bytes.startsWith('/docker') || bytes.length < 64)
		return null; // not a docker container

	const buf = Buffer.alloc(8);

	buf.write(bytes.substr(8), 0, 8, 'hex');
...
function getMacPidInstance() {
	...
	const int = interfaces.find(i => !i.internal && i.mac !== '00:00:00:00:00:00' && i.mac !== '02:42:ac:11:00:02');
