...
function getInstance() {
	...
	// Are we in the node environment?
	if (crypto && crypto.randomBytes !== void 0)
		return new Instance(Instance.schemes.RANDOM, crypto.randomBytes(8));

	// Fallback is an assumption we're in the browser environment
	const buf = new Uint8Array(8);

	return new Instance(Instance.schemes.RANDOM, crypto.getRandomValues(buf));
}
...
function getMacPidInstance() {
	try {
		...
		if (os.networkInterfaces === void 0)
			return null;
